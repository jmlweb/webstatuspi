...
#   --uninstall          Uninstall WebStatusπ
...
setup_input() {
    if [[ ! -t 0 ]]; then
        # stdin is not a terminal (piped from curl), use non-interactive mode
...
show_help() {
    cat << EOF
${BOLD}WebStatusπ Installer${RESET}
...
select_install_dir() {
    ...
    echo ""
    echo "Where would you like to install WebStatusπ?"
    echo ""
...
clone_or_update_repo() {
    step "Getting WebStatusπ source code"
...
install_dependencies() {
    ...
    info "Installing WebStatusπ in editable mode..."
    pip install -e . --quiet
    ...
    if webstatuspi --version &>/dev/null; then
        ...
        success "WebStatusπ installed: $version"
    else
...
show_completion() {
    ...
    echo ""
    echo "${GREEN}${BOLD}WebStatusπ has been installed successfully!${RESET}"
    echo ""
    ...
    if [[ "$HAS_SYSTEMD" == "true" ]] && systemctl is-active --quiet "$SERVICE_NAME" 2>/dev/null; then
        ...
        echo "  To start WebStatusπ manually:"
        echo ""
        echo "    cd $INSTALL_DIR"
        echo "    source venv/bin/activate"
...
uninstall() {
    ...
    if [[ -n "${OPT_INSTALL_DIR:-}" ]]; then
        ...
        error "No WebStatusπ installation found"
        exit 1
    fi
    ...
    if [[ "$NON_INTERACTIVE" != "true" ]]; then
        if ! ask_yes_no "Are you sure you want to uninstall WebStatusπ?" "n"; then
    ...
    echo ""
    success "WebStatusπ has been uninstalled"
    echo ""
}
...
update() {
    ...
    echo ""
    success "WebStatusπ has been updated!"
    echo ""
}
